...
exports.MyBookings = catchAsync(async (req, res, next) => {
  ...
  // '/my-bookings' in a link's href -> click it -> moving to viewRoutes why?: because it's '127.0.0.0/' so app.js will access the routes that has this paht and it's viewRoutes -> access route with '/my-bookings'.
  // Why don't use axios? -> good question bro, the answer it's simple we can use it -> Using DOM and creating new file with async function that's using axios to excute our BACKEND API Then from this function you can asign to new pug page but you can't send data except if u used another middleware with req.locals.'' = ''.
